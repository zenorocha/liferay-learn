...
| Account Number | This is the auto-generated account number. |
...
An order's status is advanced from Pending to Processing once the _Accept Order_ button is clicked.

![Orders menu - Pending tab](./orders-menu/images/09.png)

## Completed

When delivery has been confirmed, the order is moved automatically to the Completed tab.

![Orders menu - Pending tab](./orders-menu/images/08.png)

## Liferay Commerce 2.0 and below

 In previous versions of Liferay Commerce, orders are managed on the _Open_, _Pending_, and _Transmitted_ tabs according to the order’s progress through the [order life cycle](./order-life-cycle.md).
