...
The following files are required to install Liferay on the WebSphere application server:
...
The [`LIFERAY_HOME` directory](../../reference/01-liferay-home.md) is where Liferay DXP stores and manages files and folders required to function. On WebSphere, the `LIFERAY_HOME` directory is typically `[Install Location]/WebSphere/AppServer/profiles/[your-profile]/liferay`.
...
Before starting the server, verify that the following jars have been copied to the correct folders. The following files should be present within the `lib/ext` (WebSphere Application) folder:
...
1. `hsql.jar`
1. The relevant JDBC database connector jar (e.g. MySQL, MariaDB, IBM DB2, Postgres for production)
1. `portal-kernel.jar`
1. `portlet.jar`

The following folders should be present within the `[LIFERAY_HOME]/liferay/osgi` folder:
...
1. Do another filter search for the *url* property. Give this property a value that points to the database. For example, a MySQL URL would look like this:
    ...
    > **Tip:** For more example URLs, see the `jdbc.default.url` values in [Database Templates](../../reference/05-database-templates.md).
